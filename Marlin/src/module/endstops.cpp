...
void Endstops::update() {
   ...
   */
  void Endstops::set_reduced_current(const bool onoff) {
    #define HAS_CURRENT_HOME(N) (defined(N##_CURRENT_HOME) && N##_CURRENT_HOME != N##_CURRENT)
      ...
      if (onoff) {
            ...
            stepperX.rms_current(saved_current_X - REDUCE_CURRENT);
            debug_current_on(PSTR("X"), saved_current_X, saved_current_X - REDUCE_CURRENT);
          #endif
          #if HAS_CURRENT_HOME(Y)
            saved_current_Y = stepperY.getMilliamps();
            stepperY.rms_current(saved_current_Y - REDUCE_CURRENT);
            debug_current_on(PSTR("Y"), saved_current_Y, saved_current_Y - REDUCE_CURRENT);
          #endif
        #endif
        #if HAS_CURRENT_HOME(Z)
          saved_current_Z = stepperZ.getMilliamps();
          stepperZ.rms_current(saved_current_Z - REDUCE_CURRENT);
          debug_current_on(PSTR("Z"), saved_current_Z, saved_current_Z - REDUCE_CURRENT);
        #endif
        TERN_(IMPROVE_HOMING_RELIABILITY, planner.enable_stall_prevention(true));
        safe_delay(SENSORLESS_STALLGUARD_DELAY); // Short delay needed to settle     //Lujsensorless
      }
      else {
        #if ENABLED(DELTA)
          #if HAS_CURRENT_HOME(X)
            stepperX.rms_current(saved_current_X);
            debug_current_on(PSTR("X"), saved_current_X - REDUCE_CURRENT, saved_current_X);
          #endif
          #if HAS_CURRENT_HOME(Y)
            stepperY.rms_current(saved_current_Y);
            debug_current_on(PSTR("Y"), saved_current_Y - REDUCE_CURRENT, saved_current_Y);
          #endif
        #endif
        #if HAS_CURRENT_HOME(Z)
          stepperZ.rms_current(saved_current_Z);
          debug_current_on(PSTR("Z"), saved_current_Z - REDUCE_CURRENT, saved_current_Z);
        #endif
