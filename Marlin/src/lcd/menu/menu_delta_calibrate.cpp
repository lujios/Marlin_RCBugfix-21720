...
void _man_probe_pt(const xy_pos_t &xy) {
  ...
  void _goto_tower_a(const_float_t a) {
    float dcr = DELTA_PRINTABLE_RADIUS - PROBING_MARGIN;                        //Lujsensorless
    #if HAS_PROBE_XY_OFFSET                                                     //Lujsensorless
      const float total_offset = HYPOT(probe.offset_xy.x, probe.offset_xy.y);   //Lujsensorless
      dcr -= total_offset;                                                      //Lujsensorless
    #endif                                                                      //Lujsensorless
...
void lcd_delta_settings() {
  ...
  EDIT_ITEM_P(float42_52,  PSTR("Factor radius"), &sensorless_radius_factor, 0, 1);         //Lujsensorless para reducir el radio en que se aplica probing
  END_MENU();
}
