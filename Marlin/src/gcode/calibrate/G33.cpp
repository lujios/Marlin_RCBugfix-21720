...
void ac_home() {
  ...
  TERN_(SENSORLESS_HOMING, endstops.set_reduced_current(true));
  home_delta();
  TERN_(SENSORLESS_HOMING, endstops.set_reduced_current(false));
  endstops.not_homing();
}
...
void GcodeSuite::G33() {
  ...
  if (!_0p_calibration) ac_home();
    ...
    if (verbose_level > 0 && save_offset_sensorless_adj) {                  //Lujsensorless Guardar save_offset_sensorless
      ...
      probe.test_sensitivity = {true, false, false};                                      //Lujsensorless activar stallguard solo en eje x 
      if (probe_calibration_points(z_at_pt, 1, dcr, false, false, probe_at_offset)) {     //Lujsensorless Guardar save_offset_sensorless.x
      ...
      if (probe_calibration_points(z_at_pt, 1, dcr, false, false, probe_at_offset)) {     //Lujsensorless Guardar save_offset_sensorless.y
        probe.save_offset_sensorless(true, z_at_pt[CEN]);
      }      
      z_at_pt[NPP + 1] = { 0.0f };
      probe.test_sensitivity = {false, false, true};                                      //Lujsensorless activar stallguard solo en eje z
      if (probe_calibration_points(z_at_pt, 1, dcr, false, false, probe_at_offset)) {     //Lujsensorless Guardar save_offset_sensorless.z
        probe.save_offset_sensorless(true, z_at_pt[CEN]);
      }   
